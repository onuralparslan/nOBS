...
//OPTICAL EXT
static class OpSourceRoutingModuleClass : public TclClass {
public:
    OpSourceRoutingModuleClass() : TclClass("RtModule/OpSource") {}
    TclObject* create(int, const char*const*) {
        return (new OpSourceRoutingModule);
    }
} class_op_source_routing_module;
//OPTICAL EXT ENDED
...
//OPTICAL EXT
int OpSourceRoutingModule::command(int argc, const char*const* argv) {
    Tcl& tcl = Tcl::instance();
    if (argc == 3) {
        if (strcmp(argv[1] , "route-notify") == 0) {
            Node *node = (Node *)(TclObject::lookup(argv[2]));
